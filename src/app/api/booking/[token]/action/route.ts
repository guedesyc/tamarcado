...
export async function POST(request:Request,{params}:{params:Promise<{token:string}>}){
 ...
 const tokenHash=createHash("sha256").update(token).digest("hex");const {error}=parsed.data.action==="report_signal"?await supabase.rpc("report_public_booking_signal",{p_token_hash:tokenHash}):await supabase.rpc("respond_public_booking",{p_token_hash:tokenHash,p_action:parsed.data.action,p_requested_date:parsed.data.date??null,p_requested_time:parsed.data.time??null});
