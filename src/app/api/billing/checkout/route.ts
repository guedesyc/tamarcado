...
export async function POST(){
 ...
 try{
  ...
  const checkoutBody={billingTypes:["CREDIT_CARD"],chargeTypes:["RECURRENT"],minutesToExpire:60,customer:customerId,externalReference:business.id,callback:{successUrl:`${siteUrl}/app/assinatura?checkout=retorno`,cancelUrl:`${siteUrl}/app/assinatura?checkout=cancelado`,expiredUrl:`${siteUrl}/app/assinatura?checkout=expirado`},items:[{name:"Tá Marcado · Plano mensal",description:"Agenda e página profissional do Tá Marcado",quantity:1,value:29.9}],subscription:{cycle:"MONTHLY",nextDueDate}};
